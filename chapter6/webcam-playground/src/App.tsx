...
async function run() {
  try {
    ...
    const videoRef = document.getElementById('mystery') as HTMLVideoElement;
    const camDetails = await setupWebcam(videoRef);
...
async function setupWebcam(videoRef: HTMLVideoElement) {
  if (navigator.mediaDevices && 'getDisplayMedia' in navigator.mediaDevices) {
    ...
    return new Promise((resolve, _) => {
      videoRef.onloadedmetadata = () => {
        // canvasの準備
        const canvas = document.createElement('detection') as HTMLCanvasElement;
...
async function performDetections(
  ...
  camDetails: [CanvasRenderingContext2D, number, number]
) {
